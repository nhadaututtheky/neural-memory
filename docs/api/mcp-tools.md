...
List and manage domain-scoped boundaries. Boundaries are safety rules (type=boundary) that are always HOT. Use 'list' to see all boundaries grouped by domain. Use 'domains' to see unique domains with counts.
...
Brain growth milestones — track neuron-count achievements (100, 250, 500, 1000, 2500, 5000, 10000) and generate growth reports with health snapshots, velocity, and achievements. Actions: check (detect+record new milestones), progress (distance to next), history (all recorded milestones), report (force-generate current state report).
