...
| `--type / -T` | text | No | — | New memory type |
| `--content / -c` | text | No | — | New content for the anchor neuron |
| `--priority / -p` | integer | No | — | New priority 0-10 |
| `--tier` | text | No | — | New tier: hot, warm, or cold |
...
| Option | Type | Required | Default | Description |
|--------|------|----------|---------|-------------|
| `fiber_id` | text | Yes | — | (positional argument) |
| `--shared / -S` | boolean | No | `False` | Use shared/remote storage |
| `--limit / -l` | integer | No | `50` | When fiber_id='list', cap number of results |
| `--json / -j` | boolean | No | `False` | Output as JSON |
